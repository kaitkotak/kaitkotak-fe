...
import { updateFormTab } from '../../../libs/formTabService.ts'
import { FORM_TAB } from '../../../constanta/formTab.ts'
...
const PaymentForm = () => {
    ...
    const usingLocalData = localStorage.getItem(FORM_TAB.PAYMENT)

    const populateData = (data: any) => {
        getAllPaymentInvoices(data.customer_id)
        getPaymentInvoices(data.customer_id)
        form.setFieldsValue({
            ...data,
            created_at: dayjs(data.created_at),
            payment_items: data.invoice_items,
        })
    ...
    useEffect(() => {
        ...
        if (usingLocalData) {
            const data = JSON.parse(
                localStorage.getItem(FORM_TAB.PAYMENT) ?? '{}'
            )

            data.invoice_items = data.payment_items
    ...
    useEffect(() => {
        ...
        if (
            ...
            !localStorageData.payment_items
        ) {
            form.setFieldsValue({
    ...
    const calculateTotalAmount = () => {
        ...
        updateFormTab(FORM_TAB.PAYMENT, form.getFieldsValue())
    }
        ...
        >
            ...
            >
                <Form
                    ...
                    // initialValues={{
                    //     payment_items: [
                    //         { invoice_id: '', amount_due: 0, amount: 0 },
                    //     ],
                    // }}
                    onValuesChange={(_, values) => {
                        updateFormTab(FORM_TAB.PAYMENT, values)
                    }}
                >
                    <Row gutter={16}>
                        <Col span={24}>
                                        ...
                                        rules={[
                                            {
                                                required: true,
                                                message:
                                                    'Silahkan pilih tanggal pembayaran!',
                                            },
                                        ]}
                                    >
                                        <DatePicker
                                ...
                                {(fields, { add, remove }) => (
                                    <>
                                        ...
                                        <Flex gap="middle" justify="end">
                                            ...
                                            {form.getFieldValue('payment_items')
                                                ?.length <
                                                invoicesList?.length && (
                                                <Form.Item label={null}>
                                                    <Button
